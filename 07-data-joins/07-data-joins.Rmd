...
- `inner_join()`: all rows from x where there are matching values in y
...
left_join(x, y)
```
Join x and y, keeping all rows from x. Remove rows from y that do not match x. 
]
...
right_join(x, y)
```
Join x and y, keeping all rows from y. Remove rows from x that do not match y.
]
...
full_join(x, y)
```
Join x and y, keeping all rows from both x and y. Fill missing values with `NA`.
]
...
inner_join(x, y)
```
Join x and y, only keeping rows that match both x and y. Remove all other rows.
]
...
semi_join(x, y)
```
Keep all rows in x where there are matching values in y, keeping just columns from x. In other words filter x so that it contains observations that are also in y. 
]
