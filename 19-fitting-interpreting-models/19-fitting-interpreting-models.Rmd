...
- The regression line goes through the center of mass point; the coordinates correspond to average $x$ and average $y$, $(\bar{x}, \bar{y})$:  
...
- Under the hood, R creates a "contrast matrix" containing a column for each level of the categorical variable
- Each observation is coded `1` if the value matches the column name or `0` if not
---
