...
- Make subfolders for data, scripts (.qmd files), figures, and docs (reports, etc.) in the main project folder
:::
::: {.column width = "50%"}
...
## Option 1 - Manually define the column names when you read in the data using the `col_names()` argument
...
## Option 2 - Automatically format text to snake_case using the `clean_names` function (from the `janitor` package)
...
## Fix when reading in data option 1. Tell R how observations are coded as NA in your spreadsheet using the `na =` argument
...
## Fix when reading in data option 2. Specify column types using the `col_types = ` argument
...
## Write to .csv and read in tidy .csv
...
- That way, we don't have to rerun data wrangling code every time we want to work on our actual data analysis
- It also makes reading the code easier and more efficient
...
- CSVs can be unreliable for saving wrangled data if there is specific variable type information you want to keep (factors, dates, etc.).
...
## Saving and loading multiple data frames

- If your wrangling results in multiple data frames, you can save them together as an .RData file.
- You can then load them all at once using the `load()` function.

## Make two data frames and save as .RData
...
save(x, y, file = "data_processed.RData")
```
...
The data frames `x` and `y` will now be loaded as objects in your global environment.
...
# Try it: reading excel files

## ae-08 sales-excel.qmd

- Open `sales-excel.qmd`. 
