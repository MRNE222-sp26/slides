...
## Functions review: What goes in, what comes out?
...
## Write a function called `calc_mean` that, given a vector, calculates the mean of the vector. 

Remember that the mean = sum of values / number of values
...
We use the "index number" inside double brackets (`[[`) to index a list element
```{r}
a[[1]]
```
...
### First we define the `scrape_speech()` function
...
## The we use `scrape_speech()` to scrape speech info from the web
...
## We can scrape the covid-19 URLs and save them as an object
...
This is the cool part: Since the only function argument (input) we need is the URL, we can use iteration functions to automatically apply our function to each URL in our `covid_speech_urls` vector.
---
